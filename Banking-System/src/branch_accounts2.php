...
<?php
  ...
  if (checkConnectStatus()) {
    ...
    $sql = "SELECT LPAD(account, 12, 0), LPAD(customerID, 11, 0), balance, type, date_opened FROM BANK_ACCOUNT WHERE branchID = ".$bID."";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
      echo "<div class='table-wrapper'>
            <h1 class='table-title'>Branch ".$bID."'s Bank Accounts</h1>
            <table>
            <tr>
            <th>Account</th>
            <th>Customer ID</th>
